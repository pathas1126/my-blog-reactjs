...
const SignUp = props => {
  const [email_writer, setEmail_writer] = useState(false);
  const eWriterRef = useRef();

  const _signup = async () => {
    const id = document.getElementsByName("signup_id")[0].value.trim();
    const password = document
      .getElementsByName("signup_password")[0]
      .value.trim();
    const psw_check = document
      .getElementsByName("signup_pwdCheck")[0]
      .value.trim();
    const name = document.getElementsByName("signup_name")[0].value.trim();
    const birthday = document
      .getElementsByName("signup_birthday")[0]
      .value.trim();
    const sex = document.getElementsByName("signup_sex")[0].value.trim();

    // 이메일 구하기
    const email_first = document
      .getElementsByName("signup_email")[0]
      .value.trim();
    let email_select = document.getElementsByName("signup_email_select")[0]
      .value;

    if (email_select === "write") {
      email_select = document
        .getElementsByName("signup_email_write")[0]
        .value.trim();
    }
    ...
    const pw_check = /^[a-z]+[a-z0-9]{5,15}$/g;
    if (!pw_check.test(password)) {
      return alert("비밀번호는 영문자로 시작하는 6~15자여야만 합니다.");
    ...
    if (name.length === 0 || name.length < 2) {
      ...
      return alert("생년월일 숫자만 입력할 수 있습니다.");
    }

    const email_check = /^[0-9a-zA-Z]([-_.]?[0-9a-zA-Z])*@[0-9a-zA-Z]([-_.]?[0-9a-zA-Z])*.[a-zA-Z]{2,3}$/g;

    if (!email.match(email_check)) {
      return alert("올바른 이메일 형식을 입력해 주세요.");
    }

    const data = { id, password, name, birthday, sex, email };
  ...
  // email_writer 변경 함수
  const _changeEmailSelect = () => {
    const select = eWriterRef.current.value;

    if (select === "write") {
      setEmail_writer(true);
    } else {
      setEmail_writer(false);
    }
  };
  ...
  return (
    <div>
      <div>
        <h3 className="signup_title">회원가입 (Signup)</h3>
      </div>
      <div className="Signup">
        <div>
          ...
          <div>
            <h5>비밀번호 확인</h5>
            <input type="password" maxLength="15" name="signup_pwdCheck" />
          </div>
        </div>
        ...
        <div className="signup_section">
          ...
          <div>
            ...
            <select
              ref={eWriterRef}
              name="signup_email_select"
              onChange={() => _changeEmailSelect()}
            >
            ...
            {email_writer && (
              <div>
                <input type="text" name="signup_email_write" maxLength="20" />
              </div>
            )}
          </div>
        </div>
